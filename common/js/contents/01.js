	...
	var content_01 = function(){
		...
		DotLine.prototype = {
			init:function(){
				var that = this;
				var zone = this.zone;
				var dis_x = zone.ex - zone.sx;
				var dis_y = zone.ey - zone.sy;
				var dot = this.dot;

				//1つずつの距離
				dot.disX = (dis_x / (dot.len-1));
				dot.disY = (dis_y / (dot.len-1));

				//始点と終点の距離
				zone.distance = abs(dis_x);

				//移動スピード
				dot.spdX = zone.distance / dot.loop * numTimeSpd;
				dot.spdY = abs(dis_y) / dot.loop * numTimeSpd;
				dot.spdX = (zone.sx > zone.ex) ? -dot.spdX : dot.spdX;
				dot.spdY = (zone.sy > zone.ey) ? -dot.spdY : dot.spdY;

				//オブジェクト生成・設定・格納
				for(var i = 0; i<dot.len; i++){
					var d = new Dot();
					d.id = i;
					d.size = dot.size;
					d.originalSize = dot.size;
					d.spdY = dot.spdY;
					d.spdX = dot.spdX;
					d.x = (zone.sx+i*dot.disX)-dot.disX;
					d.y = (zone.sy+i*dot.disY)-dot.disY;
					d.move = zone.ex - d.x;
					this.arr[i] = d;
				}
				return false;
			...
			arcDraw:function(){
				...
				for(var i=0; i<len; i++){
					...
					var d = arr[i];
					dis = abs(d.x - that.zone.sx) - that.zone.distance;
					flg = (dis > 0) ? true : false;

					if(flg === true){
						var prev = d.id+1;
						if(prev === dot.len) prev = 0;
						d.y = arr[prev].y-dot.disY;
						d.x = arr[prev].x-dot.disX;
					}
